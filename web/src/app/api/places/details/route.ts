...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Parse address components into a flat structure
    ...
    for (const c of components) {
      // Check all types, not just types[0] — some components have multiple types
      ...
      else if (hasType('sublocality') && !city)  city = c.long_name  // fallback for some cities
