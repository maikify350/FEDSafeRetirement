...
// Displays a per-client activity timeline, fed by clientId.
// ═══════════════════════════════════════════════════════════════════
/**
 * Audit history timeline panel specific to the Client entity.
 *
 * @module D:/WIP/JobMaster_Local_Dev/web/src/views/clients/ClientHistoryPanel.tsx
 */
export default function ClientHistoryPanel({ clientId }: { clientId: string }) {
  const { data, isLoading } = useQuery({
    queryKey: ['histories', 'client', clientId],
    queryFn: () => api.get<HistoryFeedResponse>(`/api/history?clientId=${clientId}&limit=200`),
    enabled: !!clientId,
  })
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4 }}>
        ...
        <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
          No activity history for this client yet.
        </Typography>
      </Box>
    )
  }
