...
### Security / Env-Var Hardening (2026-05-05)

**Why:** `NEXT_PUBLIC_OPENAI_API_KEY` is read by four server-only API routes (`/api/quiz`, `/api/chat`, `/api/forms/[id]/summarize`, `/api/events/[id]/explainer`). The `NEXT_PUBLIC_` prefix means Next.js bundles the value into client JS, so any visitor can extract it from page source. App is internal-only today, but should be cleaned up before any public surface.

**Code already done (commits in this branch):**
- All four routes now read `OPENAI_API_KEY` first and fall back to `NEXT_PUBLIC_OPENAI_API_KEY` so a Vercel deploy can roll out without breaking until the env vars are migrated.
- `/api/events/[id]/explainer` does the same dual-read for `ELEVEN_LABS_KEY` ↔ `NEXT_PUBLIC_ELEVENLABS_API_KEY`.

**Rollout checklist (zero-downtime):**
- [ ] Push commit so Vercel has the dual-read fallback live
- [ ] In Vercel → Settings → Environment Variables: **add** `OPENAI_API_KEY` (same value, or rotated key)
- [ ] In Vercel → Settings → Environment Variables: **add** `ELEVEN_LABS_KEY` (if/when ElevenLabs is used)
- [ ] Smoke-test on the deployed site: `/training-quiz`, `/fedsafe-ai` chat, form summarize endpoint, event explainer Generate button
- [ ] In Vercel: **remove** `NEXT_PUBLIC_OPENAI_API_KEY` and `NEXT_PUBLIC_ELEVENLABS_API_KEY`
- [ ] Verify smoke tests still pass after the public vars are gone
- [ ] Local `.env`: copy values to `OPENAI_API_KEY` / `ELEVEN_LABS_KEY` and delete the `NEXT_PUBLIC_*` lines
- [ ] Once stable for a release cycle, **delete the fallback** (`|| process.env.NEXT_PUBLIC_*`) from the four routes

**Related secret cleanup (lower priority):**
- [ ] `App/scripts/add_zip_index.js` has a hardcoded Supabase DB password — refactor to read from `.env`, then rotate the DB password
- [ ] `App/scripts/create-user.mjs` and `App/scripts/run_migration.py` also contain hardcoded passwords — same treatment
- [ ] Decide whether to rotate the OpenAI key (recommended; not urgent while internal)
