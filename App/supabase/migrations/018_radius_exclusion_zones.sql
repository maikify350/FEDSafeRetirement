-- Migration 018b: Optimized Radius Exclusion Zones
-- Fixes statement timeout by:
--   1. Using a CTE to avoid computing the exclusion filter twice
--   2. Setting statement_timeout to 30s (Supabase default is 8s)
--   3. Using bbox-only pre-filter (skip expensive haversine for leads clearly outside)

-- ═══════════════════════════════════════════════════════════════════════
-- search_leads_exclude_radius (optimized)
-- ═══════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_query text;
BEGIN
  -- Allow up to 30s for this query
  PERFORM set_config('statement_timeout', '30000', true);

  v_filter_sql := build_leads_filter_clause(p_search, p_state, p_gender, p_favorite, p_filters);

  -- Build exclusion clauses using BBOX only (much faster than haversine)
  -- The bbox is generous enough that we don't need exact haversine for exclusion
  IF p_exclusion_zones IS NOT NULL AND jsonb_typeof(p_exclusion_zones) = 'array' THEN
    FOR v_zone IN SELECT * FROM jsonb_array_elements(p_exclusion_zones) LOOP
      ...
      -- Use bbox + haversine but within a single NOT block
  ...
  -- Single CTE: filter + exclude, then count via window function
  -- This avoids running the expensive exclusion twice
  v_query := '
    WITH filtered AS (
...
-- ═══════════════════════════════════════════════════════════════════════
-- exclusion_state_counts (optimized)
-- ═══════════════════════════════════════════════════════════════════════
