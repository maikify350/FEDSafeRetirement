...
interface ExportFieldPickerDialogProps {
  ...
  onExport: (rows: any[], selectedFieldKeys: string[], format: ExportFormat) => void
}
...
export default function ExportFieldPickerDialog({
  ...
  const persist = useCallback(
    (keys: string[], order: ExportField[], fmt: ExportFormat) => {
      localStorage.setItem(
        `${STORAGE_PREFIX}${storageKey}`,
        JSON.stringify({ keys, order: order.map(f => f.key), format: fmt }),
      )
    },
    [storageKey],
  )
  ...
  const deselectAll = () => {
    setSelectedKeys([])
    persist([], orderedFields, format)
  }

  const setFormatPersisted = (fmt: ExportFormat) => {
    setFormat(fmt)
    persist(selectedKeys, orderedFields, fmt)
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth='sm' fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
      ...
      <DialogContent sx={{ pt: 1 }}>
        ...
        <Box className='flex items-center gap-2 mb-3'>
          ...
          <Chip
            ...
            onClick={() => setFormatPersisted('json')}
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        >
          Export {rows.length.toLocaleString()} {format === 'xlsx' ? 'Excel' : format.toUpperCase()}
        </Button>
      </DialogActions>
    </Dialog>
  )
