...
export default function FedSafeAIPanel({ open, onClose }: FedSafeAIPanelProps) {
  ...
  const [suggestedQuestions, setSuggestedQuestions] = useState<string[]>(ALL_QUESTIONS.slice(0, 4))
  ...
  // Randomize questions on client only to avoid hydration mismatch
  useEffect(() => {
    setSuggestedQuestions(pickRandom(ALL_QUESTIONS, 4))
  }, [openCount])
