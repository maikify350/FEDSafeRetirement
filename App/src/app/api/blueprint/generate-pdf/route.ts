...
PDF_Preparer_API.loadDocumentFieldMap = function (mapFileName: string) {
  ...
  const mapPath = path.join(process.cwd(), 'public', 'pdfmaps', path.basename(mapFileName))
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isXfa || isNoFields) {
      ...
      // Embed Carlito Regular + Bold (metrically identical to Calibri).
      // Use per-field bold detection so bold fields keep their weight.
      const carlito  = await pdfDoc.embedFont(readFileSync(resolve(process.cwd(), 'src/lib/pdfgen/Carlito-Regular.ttf')))
      const carlitoB = await pdfDoc.embedFont(readFileSync(resolve(process.cwd(), 'src/lib/pdfgen/Carlito-Bold.ttf')))
