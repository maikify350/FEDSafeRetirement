...
return NextResponse.json({ error: error.message }, { status: 500 })
  ...
  // Helper: apply all shared filters (state, search, column filters) to a query
  const applyFilters = (q: any) => {
    // Apply state filter if present
    if (stateParam) {
      q = q.eq('facility_state', stateParam)
    }
  ...
  if (sorting.length > 0) {
    for (const s of sorting) {
      dataQuery = dataQuery.order(s.id, { ascending: !s.desc })
    }
  } else {
...
return NextResponse.json({ error: dataResult.error.message }, { status: 500 })
  ...
  const total = countResult.error ? (dataResult.data?.length ?? 0) : (countResult.count ?? 0)
