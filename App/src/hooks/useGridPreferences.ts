...
export function useGridPreferences(
  ...
  // Start with defaults so SSR and the first client render agree (avoids
  // hydration mismatch). Read localStorage in an effect after mount.
  const [prefs, setPrefsState] = useState<GridPrefs>(defaultPrefs)
