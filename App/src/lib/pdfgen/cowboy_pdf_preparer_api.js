...
const PDF_Preparer_API = {
    // --- 0. REDUCTION NORMALIZER ---
    // ACT CRM combo-box sends values like "1 - 75%", "2 - 50%", "3 - 0%".
    // This helper extracts the canonical percentage string: "75", "50", or "0".
    // Always use this before comparing feglireduction values.
...
return values.map((value) => value.trim());
    ...
    loadDocumentFieldMap: function(mapFileName) {
        // path.basename strips any ../ segments — defense-in-depth against traversal.
        const mapPath = path.join(__dirname, "..", "Resources", "unsorted", path.basename(mapFileName));
...
return parts.length > 1 && parts.every((part) => addressTails.has(this.getPathTail(part)));
    ...
    resolvePrefillToken: function(actJson, token, pdfField, formKey) {
        ...
        const reduction = this.normalizeReduction(customFields.feglireduction);
...
return Math.floor((yearsValue * 12) + 0.000001);
    ...
    buildNotesField: function(customFields) {
        ...
        const dynamicKeys = Object.keys(customFields || {})
    ...
    buildCrmUpdatePayload: function({
        ...
        return {
            customFields: {
                ...
                calculatedgrossbridgemonthly: (bridgeActive && roundedGrossBridge > 0) ? this.formatCrmWholeNumber(roundedGrossBridge) : "",
                calculatednetbridgemonthly: (bridgeActive && roundedNetBridge > 0) ? this.formatCrmWholeNumber(roundedNetBridge) : "",
...
return map[safeBMult][col];
    ...
    calculateRetireeFEGLI: function({ salary, age, reduction, hasA, bMult, cMult }, annuitantRates) {
        ...
        const normReduction = this.normalizeReduction(reduction);
        const reductionSuffix = normReduction;
        const basicKey = `basic_${reductionSuffix}`;
        const reductionPercent = normReduction === "0" ? 1.0 : (normReduction === "50" ? 0.5 : 0.25);
        const safeBMult = Math.min(5, Math.max(0, parseInt(bMult, 10) || 0));
        const safeCMult = Math.min(5, Math.max(0, parseInt(cMult, 10) || 0));

        // OPM Basic Insurance Amount (BIA) = rounded-salary + $2,000 (bia.total).
        // The retirement coverage and its premium must be based on bia.total (matching
        // calculateActiveFEGLI), so that a 75% reduction correctly yields 25% of bia.total
        // (e.g. $94,000 * 0.25 = $23,500), not 25% of bia.base ($92,000 * 0.25 = $23,000).
        return {
    ...
    buildFegliResult: function(actJson, empRateInput, annuitantRateInput) {
        ...
        const retireAge = parseInt(c.feglicostage, 10) || this.ageOnDate(actJson.birthday, retirementDate);
        ...
        const reduction = this.normalizeReduction(c.feglireduction);
...
return this.buildFegliOnlyResponse(actJson, fegliResult);
    ...
    execute: function(actJson, empCsv, annuitantCsv, federalTaxCsv, stateTaxCsv, stateRetirementTaxRulesInput) {
        ...
        const boughtTimeBack = this.parseBoolean(c.boughttimeback);
        const militaryMonths = boughtTimeBack ? this.convertYearsToWholeMonths(militaryTime) : 0;
        ...
        output["Military Time"] = militaryTime > 0 ? formattedMilitaryTime : "";
        output["Sick Leave"] = sickLeaveHours > 0 ? this.formatCount(sickLeaveHours, 0) : "";
        ...
        output["less FEGLI"] = (retireIsIneligible || retireIsWaived)
            ? (ltcInsuranceMonthly > 0 ? this.formatAccountingCurrency(ltcInsuranceMonthly) : "N/A")
            : this.formatAccountingCurrency(monthlyFegli + ltcInsuranceMonthly);
        ...
        output["Age 62 Social Security"] = bridgeActive ? this.formatCurrency(grossSocialSecurityInput) : "";
        ...
        output["Projected Social Security"] = projectedSsComponent > 0 ? this.formatCurrency(projectedSsComponent) : "";
        ...
        output.crmUpdatePayload = this.buildCrmUpdatePayload({
            ...
            socialSecurityGross,
            netSocialSecurity,
            age62SocialSecurityEstimate: grossSocialSecurityInput,
            grossBridge,
            netBridge,
            tspTraditional,
            tspRoth,
    ...
    executeTemplate2: function(actJson, empCsv, annuitantCsv, federalTaxCsv, stateTaxCsv, stateRetirementTaxRulesInput) {
        ...
        const opmService = this.calculateOPMService(c.servicecomputationdate, retirementDate);
        const militaryTime = this.toNumber(c.yrsofmilitaryservice);
        const boughtTimeBackT2 = this.parseBoolean(c.boughttimeback);
        const militaryMonths = boughtTimeBackT2 ? this.convertYearsToWholeMonths(militaryTime) : 0;
        const sickLeaveHours = this.getSickLeaveHours(c);
        const sickLeaveMonths = Math.floor(sickLeaveHours / 174);
        const serviceMonthsWithMilitary = opmService.totalMonths + militaryMonths;
        const serviceMonthsWithSickLeave = serviceMonthsWithMilitary + sickLeaveMonths;
        ...
        output["Sick Leave"] = sickLeaveHours > 0 ? this.formatTemplate2HoursDisplay(sickLeaveHours) : "";
        ...
        output["Bridge Years of Service"] = bridgeActive
            ...
            : "";
        output["Military Time"] = militaryTime > 0 ? this.formatTemplate2DurationFromYears(militaryTime) : "";
